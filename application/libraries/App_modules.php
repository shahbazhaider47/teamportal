...
class App_modules
{
    ...
    public function __construct()
    {
        ...
        $this->ci->load->helper('file'); // ✅ This is required for read_file()
         ...
         */
        if (!$this->ci->db->table_exists('tblmodules')) {
            return;
        }
    ...
    public function activate($name)
    {
        ...
        if (!$this->module_exists_in_database($name)) {
            ...
            $this->ci->db->where('module_name', $name)->update('tblmodules', [
                'active' => 1,
            ]);
        }
    
        // Load the init file manually if not already
    ...
    public function deactivate($name)
    {
        ...
        $this->ci->db->where('module_name', $name)->update('tblmodules', ['active' => 0]);
    ...
    public function uninstall($name)
    {
         ...
         * Delete module files
         */
        $this->delete_module_directory($name);
...
{
    // Ensure the migration class is loaded
    ...
    public function get_database_module($name)
    {
        ...
        return $this->ci->db->get('tblmodules')->row();
    }
    ...
    foreach (static::get_valid_modules() as $module) {
        ...
        if (!file_exists($module['init_file'])) {
            log_message('error', "❌ Init file not found for module: {$name} at {$module['init_file']}");
            continue;
        }
        
        $headers = $this->get_headers($module['init_file']);
        
        // 💥 Check if headers contain essential metadata
        if (!isset($headers['module_name']) || !isset($headers['version'])) {
            log_message('error', "❌ Invalid module headers in: {$name}");
            continue;
        }
        
        // 🔄 Store validated module
        $this->modules[$name] = [
            'system_name'       => $name,
            'headers'           => $headers,
            'init_file'         => $module['init_file'],
            'path'              => $module['path'],
        ];

    
        // 💡 Ensure module is tracked in the database
        if (!$this->module_exists_in_database($name)) {
            $this->ci->db->insert('tblmodules', [
                'module_name'       => $name,
                'installed_version' => $this->modules[$name]['headers']['version'],
                'active'            => 0,
            ]);
        }
    
        $moduleDB = $this->get_database_module($name);
        $isActive = $moduleDB && (int) $moduleDB->active === 1;
    
        $this->modules[$name]['activated']         = $isActive ? 1 : 0;
        ...
        if ($isActive) {
            ...
            if (file_exists($module['init_file'])) {
                ...
                log_message('error', "Module init file missing for: {$name} at path {$module['init_file']}");
            }
        }
    ...
    public function get_headers($module_source)
    {
        ...
        $module_data = read_file($module_source);
    ...
    {
        $db_modules = $this->ci->db->get('tblmodules')->result();
    ...
    /**
 * List all modules in filesystem, regardless of activation status.
 * Returns metadata like name, version, description, etc.
 */
public function get_all_filesystem_modules()
{
    $modules = [];

    $folders = directory_map(APP_MODULES_PATH, 1);
    ...
    foreach ($folders as $folder) {
        ...
        $modulePath = APP_MODULES_PATH . $folder;
        $initPath   = $modulePath . '/' . $folder . '.php';

        if (!file_exists($initPath)) {
            $initPath = $modulePath . '/' . ucfirst($folder) . '.php';
            if (!file_exists($initPath)) {
                continue;
            }
        }

        $headers = $this->get_headers($initPath);
        $modules[] = [
            'system_name' => $folder,
            'headers'     => $headers,
            'path'        => $modulePath,
            'init_file'   => $initPath,
            'active'      => $this->is_active($folder),
        ];
    }
...
protected function delete_module_directory($name)
{
    $path = APP_MODULES_PATH . $name;
...
{
    return array_column($this->modules, 'path', 'system_name');
}
...
{
    $modules = $this->get_all_filesystem_modules(); // Scans module folders

    // Ensure DB modules are loaded
    $this->query_db_modules();

    // Merge DB activation state into each module
    foreach ($modules as $key => &$mod) {
    $slug = $mod['system_name'];
    $mod['active'] = isset($this->db_modules[$slug]) ? (bool) $this->db_modules[$slug]->active : false;

    }
    
    log_message('debug', 'Merged Modules: ' . print_r($modules, true));
    return $modules;
}

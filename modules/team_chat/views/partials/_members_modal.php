...
<script>
(function () {
    const modal        = document.getElementById('tcMembersModal');
    const addSection   = document.getElementById('tcMembersAddSection');
    const addToggleBtn = document.getElementById('tcToggleAddMember');
    const searchInput  = document.getElementById('tcAddMemberSearch');
    const resultsEl    = document.getElementById('tcAddMemberResults');

    if (!modal) return;

    let searchTimer = null;

    // When modal opens: JS (team_chat_members.js) reloads fresh member list
    // and shows admin actions if current user is owner/admin
    modal.addEventListener('show.bs.modal', function () {
        if (typeof TeamChat !== 'undefined') {
            TeamChat.refreshMembersModal();
        }
    });
    ...
    if (addToggleBtn) {
        addToggleBtn.addEventListener('click', function () {
            const isOpen = !addSection.classList.contains('d-none');
            addSection.classList.toggle('d-none', isOpen);
            addToggleBtn.innerHTML = isOpen
    ...
    function _searchUsers(q) {
        fetch(window.TeamChatConfig.baseUrl + '/users/search?q=' + encodeURIComponent(q), {
        ...
        .then(res => {
            ...
            resultsEl.innerHTML = res.data.map(u => `
                <div class="tc-user-result" data-user-id="${u.id}" data-name="${_esc(u.fullname)}">
                    ...
                    <button class="btn btn-sm btn-primary ms-auto tc-add-single-member"
                            data-uid="${u.id}">
            ...
            resultsEl.querySelectorAll('.tc-add-single-member').forEach(btn => {
                btn.addEventListener('click', function () {
                    const uid = parseInt(this.dataset.uid);
                    this.disabled = true;
                    this.innerHTML = '<span class="spinner-border spinner-border-sm"></span>';
                    TeamChat.addMember(uid).then(() => {
    ...
    document.addEventListener('click', function (e) {
        const btn = e.target.closest('.tc-role-change');
        if (!btn) return;
        e.preventDefault();
        const uid  = parseInt(btn.dataset.uid);
        const role = btn.dataset.role;
        TeamChat.updateMemberRole(uid, role);
    });

    // Remove member
    document.addEventListener('click', function (e) {
        const btn = e.target.closest('.tc-remove-member');
        if (!btn) return;
        const uid  = parseInt(btn.dataset.uid);
        const name = btn.dataset.name;
        if (confirm('Remove ' + name + ' from this conversation?')) {
            TeamChat.removeMember(uid);
        }
    });

    // Leave conversation
    document.addEventListener('click', function (e) {
        const btn = e.target.closest('.tc-leave-conv');
        if (!btn) return;
        if (confirm('Are you sure you want to leave this conversation?')) {
            TeamChat.leaveConversation();
        }
    });

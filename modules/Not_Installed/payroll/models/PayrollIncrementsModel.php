...
public function users_dropdown(): array
{
    $this->db->from('users AS u')
             ->select('u.id, u.emp_id, u.firstname, u.lastname, u.current_salary');

    // Use the base helper to match your schema (is_active / active / status)
...
public function departments_dropdown(): array
{
     ...
     */
    public function users_for_scope(string $scope, array $user_ids = [], int $department_id = 0, int $position_id = 0): array
    {
        $qb = $this->db->select('u.id, u.emp_id, u.firstname, u.lastname, u.current_salary')
                       ->from('users AS u');
    
        // Active filter that adapts to your schema
        if (method_exists($this, 'where_active_users')) {
            $this->where_active_users();
        } else {
            $this->db->where('u.is_active', 1);

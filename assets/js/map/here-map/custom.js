...
var map = new H.Map(document.getElementById('map1'), defaultLayers.normal.map, { pixelRatio: pixelRatio });
...
var map = new H.Map(document.getElementById('map2'), defaultLayers.normal.map, { pixelRatio: pixelRatio });
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
var ui = H.ui.UI.createDefault(map, defaultLayers);
...
function switchMapLanguage(map, platform) {
  var mapTileService = platform.getMapTileService({
    ...
    chineseMapLayer = mapTileService.createTileLayer('maptile', 'normal.day', pixelRatio === 1 ? 256 : 512, 'png8', { lg: 'CHI', ppi: pixelRatio === 1 ? undefined : 320 });
...
function switchMapType(map, platform) {
  ...
  terrainMap = aerialMapTileService.createTileLayer('maptile', 'terrain.day', pixelRatio === 1 ? 256 : 512, 'png8', { ppi: pixelRatio === 1 ? undefined : 320 });
...
function renderControls(title, buttons) {
  ...
  containerNode.innerHTML = '</h4><div class="btn-group"></div>';
  containerNode.setAttribute('style', 'position:absolute;top:0;left:0;background-color:#fff; padding:10px;text-align:center');
  Object.keys(buttons).forEach(function (label) {
    var input = document.createElement('input');
    input.value = label;
    input.type = 'button';
    input.onclick = buttons[label];
    input.className = 'btn btn-sm btn-primary text-white';
...
var map = new H.Map(document.getElementById('map10'), defaultLayers.normal.map, {
...
var map = new H.Map(document.getElementById('map11'), defaultLayers.satellite.map, {
  center: { lat: 0, lng: 0 },
  zoom: 1,
  pixelRatio: pixelRatio,
});
...
var map = new H.Map(document.getElementById('map12'), defaultLayers.normal.map, {
  center: { lat: 50, lng: 5 },
  zoom: 4,
  pixelRatio: pixelRatio,
});
...
var map = new H.Map(document.getElementById('map14'), defaultLayers.normal.map, {
  center: { lat: 52, lng: 5 },
  zoom: 5,
  pixelRatio: pixelRatio,
});
...
function addPolygonToMap(map) {
  var lineString = new H.geo.LineString([52, 13, 100, 48, 2, 100, 48, 16, 100, 52, 13, 100], 'values lat lng alt');
  map.addObject(
...
var map = new H.Map(document.getElementById('map15'), defaultLayers.normal.map, {
  center: { lat: 52, lng: 5 },
  zoom: 5,
  pixelRatio: pixelRatio,
});
...
var map = new H.Map(document.getElementById('map16'), defaultLayers.normal.map, {
